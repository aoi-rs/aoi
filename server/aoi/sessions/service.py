...
from aoi.kit.db.locking import is_lock_not_available_error
from aoi.kit.pagination import PaginationParams
from aoi.kit.utils import utc_now
from aoi.models import Session, User
...
class RefreshTokenAlreadyUsedError(AoiError):
    def __init__(self):
...
class SessionService:
    async def list(
        ...
        return await repository.paginate(statement, pagination)
    ...
    async def create(
        ...
        user_session = Session(
            ...
            refresh_token_hmac_key=base64.urlsafe_b64encode(
    ...
    async def refresh(self, request: Request, session: AsyncSession):
        ...
        while (
            ...
            if counter_difference < 0:
                raise AoiError(
                    "This refresh token was not issued by this server", status_code=400
                )
                ...
                reuse_allowed = (
                    likely_not_saved_by_client or likely_concurrent_refreshes
                )
