...
async def login_callback(
  ...
  access_token_state: tuple[OAuth2Token, str | None] = Depends(oauth2_login_authorize_callback),
  redis: Redis = Depends(get_redis)
) -> RedirectResponse:
  token_data, state = access_token_state
  state_data = await validate_callback(request, redis, token_data, state, type="google")

  return_to = state_data.get("return_to")

  try:
    user, = await google_service.get_updated_or_create(
      session,
        token=token_data,
    )
