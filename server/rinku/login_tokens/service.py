...
class LoginTokenService:  
  async def request(self, session: AsyncSession, email: str):
    ...
    login_token = LoginToken(email=email, token_hash=token_hash, expires_at=expires_at)
  ...
  async def check(
    ...
    if (
      ...
      raise RinkuError("This login token is invalid or has expired", status_code=401)
  ...
  def _generate_token_hash(self, token: str) -> str:
    result = hmac.new(settings.SECRET.encode("ascii"), token.encode("ascii"), hashlib.sha256)
